...
// Two-step SMS-based password reset (backend/CLAUDE.md's "Auth & roles" section, via
// the shared Termii account — see forgot-password.dto.ts/reset-password.dto.ts on the
// backend). Step 1 always "succeeds" from the caller's point of view whether or not
// the phone number has an account (AuthService.forgotPassword()'s anti-enumeration
// design), so the UI can't and shouldn't distinguish those cases either.
